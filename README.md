...
- `BayesianLinearStat`: Bayesian linear regression with a full-covariance Gaussian posterior `N(μ, Σ)` over weights. Conjugate updates per `(features, target)` observation using the Sherman-Morrison rank-1 formula, `read().mean` / `read().covariance` for the posterior, `read().sampleWeights()` for Thompson-sampling draws, `read().predict(x)` for the point prediction, `read().posteriorPredictive(x)` returning a `NormalReading` instead of a scalar. Mergeable across observation streams (well-defined for conjugate Gaussian posteriors). Primary consumer: [klause](https://github.com/eignex/klause) for online value/variable selection in CP search via LinUCB / Linear Thompson Sampling; useful standalone for online experiment analysis where a point estimate is not enough.
- `BayesianLogisticStat`: binary classification companion with a Laplace-approximated Gaussian posterior over weights. Same surface as `BayesianLinearStat` but `predict(x)` returns a probability and updates are observation = `(features, 0/1)`. Klause uses this for "did this (var, value) branch lead to a feasible model?" learners. Update cost is O(d²) per observation; for d ≤ ~100 this is fast enough for hot paths.
- Hierarchical Bayesian wrapper: per-stream prior shared across instances. `HierarchicalBayesianLinearStat` keeps a class-level posterior plus per-instance posteriors; new instances start at the class mean. The mergeable-snapshot pattern handles the cross-instance transfer naturally.
