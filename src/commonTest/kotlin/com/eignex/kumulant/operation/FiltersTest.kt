...
class FiltersTest {
    ...
    fun `series filter gates updates by predicate`() {
        val stat = SumStat().filter { it > 0.0 }
        stat.update(-1.0)
        stat.update(2.0)
        stat.update(3.0)
    ...
    fun `series filter create preserves predicate`() {
        val template = SumStat().filter { it > 0.0 }
    ...
    fun `paired filter gates updates`() {
        val stat = SumStat().atX().filter { x, y -> x + y > 0.0 }
    ...
    fun `discrete filter gates updates`() {
        val stat = HyperLogLogStat(precision = 10).filter { it >= 0L }
    ...
    fun `discrete filter create preserves predicate`() {
        val template = LinearCountingStat(bits = 1024).filter { it % 2L == 0L }
        val fresh = template.create()
    ...
    fun `paired filter rejecting all leaves stat in zero state`() {
        val stat = SumStat().atX().filter { _, _ -> false }
        stat.update(1.0, 2.0)
        stat.update(3.0, 4.0)
        assertEquals(0.0, stat.read().sum, DELTA)
    }
    ...
    fun `vector filter rejecting all leaves stat in zero state`() {
        val stat = SumStat().atIndex(0).filter { _ -> false }
    ...
    fun `series filter excludes NaN values`() {
        val stat = SumStat().filter { !it.isNaN() }
        stat.update(1.0)
    ...
    fun `paired filter excludes pairs containing NaN`() {
        val stat = SumStat().atY().filter { x, y -> !x.isNaN() && !y.isNaN() }
    ...
    fun `vector filter excludes vectors containing NaN`() {
        val stat = SumStat().atIndex(0).filter { v -> v.none { it.isNaN() } }
    ...
    fun `series filter passes all when predicate always true`() {
        val stat = com.eignex.kumulant.stat.summary.MeanStat().filter { true }
        stat.update(1.0)
        stat.update(3.0)
