 ...
 *
 * This is the core primitive for all time-decaying statistics. The sum
 * continuously decays toward zero as time passes, giving recent observations
 * more influence than older ones.
 *
 * [halfLife] is the time after which any contribution halves in weight.
 * α = ln(2) / halfLife.
 */
class DecayingSum(
    ...
    // The accumulator stores energy referenced to landmarkNanos.
    // S(t) = accumulator * exp(−α * (t − landmarkNanos))
 ...
 *
 * Built on [DecayingSum]. Prefer [DecayingEventRate] for pure event counting.
 */
class DecayingRate(
    val halfLife: Duration,
    val mode: StreamMode = defaultStreamMode,
 ...
 * The ratio gives the mean of the exponentially windowed distribution.
 ...
 * Tracks how spread changes as the recent signal shifts.
 */
